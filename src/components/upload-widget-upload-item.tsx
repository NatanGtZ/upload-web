...
export function UploadWidgetUploadItem({upload, uploadId} : UploadWidgetUploadItemProps) {
  ...
  return (
    //[&+div]:mt-2 coloca um margin-top entre os elementos, mas não entre o primeiro e o segundo
    <motion.div 
    ...
    >
      <div className="flex flex-col gap-1">
        <span className="text-xs font-medium flex items-center gap-1">
          ...
          <span className="max-w-[180px] truncate overflow-hidden">{upload.name}</span>
        </span>
      ...
      <div className="absolute top-2 right-2 flex items-center gap-1">
        <Button size="icon-sm" arai-disabled={!upload.remoteUrl} onClick={() => downloadUrl(upload.remoteUrl!)}>
            <Download className="size-4" strokeWidth={1.5}/>
            <span className="sr-only">Download compressed image</span>
        </Button>

        <Button size="icon-sm" 
          disabled={!upload.remoteUrl} onClick={() => upload.remoteUrl && navigator.clipboard.writeText(upload.remoteUrl)}>
